...
// 构建状态(位置、姿态、lidar到imu外参、地面系下速度、零偏、重力向量、底盘到imu外参)
MTK_BUILD_MANIFOLD(state_ikfom,
				   ((vect3, pos))((SO3, rot))((SO3, offset_R_L_I))((vect3, offset_T_L_I))
				   ((vect3, vel))((vect3, bg))((vect3, ba))((S2, grav))((SO3,offset_R_V_I))((vect3,offset_T_V_I)));
// 构建输入(加速度、角速度)
...
MTK::get_cov<process_noise_ikfom>::type process_noise_cov()
{
	...
	MTK::setDiagonal<process_noise_ikfom, vect3, 0>(cov, &process_noise_ikfom::ng, 0.1);	  // 0.03
	MTK::setDiagonal<process_noise_ikfom, vect3, 3>(cov, &process_noise_ikfom::na, 0.1);	  // *dt 0.01 0.01 * dt * dt 0.05
	MTK::setDiagonal<process_noise_ikfom, vect3, 6>(cov, &process_noise_ikfom::nbg, 0.0001); // *dt 0.00001 0.00001 * dt *dt 0.3 //0.001 0.0001 0.01
	MTK::setDiagonal<process_noise_ikfom, vect3, 9>(cov, &process_noise_ikfom::nba, 0.0001); // 0.001 0.05 0.0001/out 0.01
