  ...
  if (feature_enabled)
  {
    ...
    for (uint i = 1; i < plsize; i++)
    {
      if ((msg->points[i].line < N_SCANS) && ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00))
      {
        ...
        // msg->points[i].offset_time：offset time relative to the base time
...
void Preprocess::velodyne_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  pcl::fromROSMsg(*msg, pl_orig);
  double min_ap, max_ap, min_ay, max_ay;
  // cout<<endl;
  for(auto i=0;i<pl_orig.size();i++){
    // if(i%1000==0){
    //   cout<<pl_orig.points[i].time<<",";
    // }
    // cout<<atan2(pl_orig.points[i].y, pl_orig.points[i].x) * 57.29578<<",";
    auto angle_p = atan2(pl_orig.points[i].z,sqrt(pow(pl_orig.points[i].x,2)+pow(pl_orig.points[i].y,2)))* 57.29578;
    pl_orig.points[i].ring = int(round( (angle_p+15)/2+1 ));
    // auto angle_y = atan2(pl_orig.points[i].y, pl_orig.points[i].x)* 57.29578;
    // auto rotate_angle = 90-angle_y;
    // if(rotate_angle<0){
    //   rotate_angle = rotate_angle+360.0;
    // }
    // if(i==0){
    //   cout<<"start rotate: "<<rotate_angle<<endl;
    // }
    // if(i==pl_orig.size()-1){
    //   cout<<"end rotate: "<<rotate_angle<<endl;
    // }


    // pl_orig.points[i].time = dt* ( rotate_angle /360.0);
  //   if(angle_p<min_ap){
  //     min_ap = angle_p;
  //   }
  //   if(angle_p>max_ap){
  ...
  // cout<<"SIZE is: "<<plsize<<endl;
  // cout<<"min_anglep: "<<min_ap<<" max_anglep: "<<max_ap<<endl;
  // cout<<"min_angley: "<<min_ay<<" max_angley: "<<max_ay<<endl;
  ...
  {//ROS_WARN("%f",pl_orig.points[plsize - 1].time);
  ...
  else
  {
    ...
    for (int i = 0; i < plsize; i++)
    {
      ...
      {
        int layer = pl_orig.points[i].ring;
        double yaw_angle = atan2(added_pt.y, added_pt.x) * 57.29578;
