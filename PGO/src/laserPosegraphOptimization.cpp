...
string KeyFramePath = "/home/i/VLIO/src/VLIO/PGO/keyframes";
...
pcl::PointCloud<PointType>::Ptr laserCloudMapSave(new pcl::PointCloud<PointType>());
...
void saveKeyFrame(pcl::PointCloud<PointType>::Ptr keyframe, Pose6D pose, int id){
    auto keyframeworld = local2global(keyframe, pose);
    auto pcd_path = KeyFramePath+"/Scans/"+to_string(id)+".pcd";
    pcd_writer.writeBinary(pcd_path,*keyframe);
    writeTxt<<pose.x<<","<<pose.y<<","<<pose.z<<","<<pose.roll<<","<<pose.pitch<<","<<pose.yaw<<endl;
}
void process_pg()
{
    while(1)
    {
		while ( !odometryBuf.empty() && !fullResBuf.empty() )
        {
            ...
            if( translationAccumulated > keyframeMeterGap || rotaionAccumulated > keyframeRadGap ) {
                ...
                rotaionAccumulated = 0.0; // reset writeTxt.open(KeyFrameTxtPath,ios::app|ios::out);
            } else {
                isNowKeyFrame = false;
            }
...
void saveMap()
{
    laserCloudMapSave->clear();
    for (int i=0; i < keyframeLaserClouds.size(); i++) {
        *laserCloudMapSave += *local2global(keyframeLaserClouds[i], keyframePosesUpdated[i]);
    }
    auto map_path = save_directory+"map.pcd";
    pcd_writer.writeBinary(map_path,*laserCloudMapSave);
    cout<<"save map in: "<<map_path<<endl;
}
int main(int argc, char **argv)
{
	...
	pubOdomRepubVerifier = nh.advertise<nav_msgs::Odometry>("/repub_odom", 100);// 没用
	...
	//std::thread viz_path {process_viz_path}; // visualization - path (high frequency)
     ...
     auto unused2 = system((std::string("exec rm -r ") + KeyFramePath+"/Scans").c_str());
    unused2 = system((std::string("mkdir -p ") +  KeyFramePath+"/Scans").c_str());
     writeTxt.open(KeyFrameTxtPath,ios::out);
     for(int i=0;i<keyframePoses.size();i++){
        saveKeyFrame(keyframeLaserClouds.at(i),keyframePosesUpdated.at(i),i);
     }
    cout<<"save scan in: "<<KeyFramePath+"/Scans"<<endl;
    writeTxt.close();
    saveMap();
	return 0;
}